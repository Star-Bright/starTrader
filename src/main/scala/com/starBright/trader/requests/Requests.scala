package com.starBright.trader.requests
...
object Requests {
  ...
  case class FundamentalData(reqId: Int, contract: Contract, reportType: String, list: util.List[TagValue])
      extends IBRequest
  ...
  case class HistoricalData(
    ...
    useRTH: Int,
    formatDate: Int,
    chartOptions: util.List[TagValue]
  ) extends IBRequest

  case class AccountUpdates(subscribe: Boolean, acctCode: String) extends IBRequest

  case class MktDepth(tickerId: Int, contract: Contract, numRows: Int, mktDepthOptions: util.ArrayList[TagValue])
      extends IBRequest

  case class SecDefOptParams(
    reqId: Int,
  ...
  case class RealTimeBars(
    tickerId: Int,
    contract: Contract,
    barSize: Int,
    whatToShow: String,
    useRTH: Boolean,
    realTimeBarsOptions: util.ArrayList[TagValue]
  ) extends IBRequest

  case class Ids(numIds: Int) extends IBRequest
  case object CurrentTime extends IBRequest

  case class CalculateImpliedVolatility(
    reqId: Int,
    contract: Contract,
    optionPrice: Double,
    underPrice: Double,
    list: util.List[TagValue]
  ) extends IBRequest

  case class MktData(i: Int, contract: Contract, s: String, b: Boolean, b1: Boolean, list: util.List[TagValue])
      extends IBRequest
  ...
  case class AccountUpdatesMulti(reqId: Int, account: String, modelCode: String, ledgerAndNLV: Boolean)
      extends IBRequest
  ...
  case class CalculateOptionPrice(
    reqId: Int,
    contract: Contract,
    volatility: Double,
    underPrice: Double,
  ...
  case class ScannerSubscription(
    tickerId: Int,
    subscription: client.ScannerSubscription,
    list: util.List[TagValue],
    scannerSubscriptionOptions: util.ArrayList[TagValue]
  ) extends IBRequest
