...
import com.starbright.trade.ibclient.contract.StockContract
...
//  private[this] val cfg = ConfigFactory
//    .load()
...
//   * @param barSize
//   * @return
//   *   the maximum number of bar (duration) that can be requested with the given parameters or None if the combination
//   *   is not valid.
//   */
...
//  }
  ...
  def bestDuration(startDate: LocalDate, endDate: LocalDate, barSize: BarSize): HistoryDuration = {
  ...
  /**
   * @author
   *   piotr 01.03.15 History limits are not clearly documented, this class can automatically find the limits via
   *   calibrate
   */
  class HistoryLimits {
    ...
    def testWaitDuration: FiniteDuration = scala.concurrent.duration.Duration(config.tws.timeout, SECONDS)

    def testStockContract: StockContract =
      new StockContract(
        config.asset.stock.contract,
        config.asset.stock.exchange,
        config.asset.stock.currency
      )

    def connectedClient: IBClient = {
      val ibclient = new IBClient(config.tws.host, config.tws.port, config.tws.clientId)
    ...
    def validDuration(barSize: BarSize, durationUnit: DurationUnit, duration: Int): Boolean = {
      ...
      val futureBars =
        ibclient.historicalData(contract, endDate, duration, durationUnit, barSize, WhatToShow.TRADES, false)
      Thread.sleep(16000)
    ...
    def findLimits(barsizes: Array[BarSize], durations: Array[DurationUnit]): Seq[ValidDurations] = {
      implicit class Crossable[X](xs: Traversable[X]) {
        def cross[Y](ys: Traversable[Y]) = for {
          x <- xs
          y <- ys
        } yield (x, y)
      }
    ...
    def calibrate(): Seq[ValidDurations] = {
      val barsizes =
        Array[BarSize](BarSize._15_mins, BarSize._1_hour, BarSize._4_hours, BarSize._1_day, BarSize._1_week)
