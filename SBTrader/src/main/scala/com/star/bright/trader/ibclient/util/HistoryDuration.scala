...
case class HistoryDuration(total: Int, eachRequest: Int, durationUnit: DurationUnit) {
  ...
  /**
   * @return
   *   number of requests to perform with the available parameters
   */
   ...
   * @return
   *   a vector of dates to request
   */
  def endDates(endDate: LocalDate): Vector[Date] = {
  ...
  /**
   * @return
   *   a vector of durations to request from past to present
   */
