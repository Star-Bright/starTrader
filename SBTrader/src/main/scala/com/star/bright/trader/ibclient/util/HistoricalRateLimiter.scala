package com.starbright.trade.ibclient.util
...
/**
 * Stateful utility to account for historical requests in order no to violate historical data limitations as specified
 * in: https://www.interactivebrokers.com/en/software/api/apiguide/tables/historical_data_limitations.htm
 *
 * The basic usage is done through the following calls:
 *
 * Requests should be accounted for with the "requested" method nextRequestAfter_ms gives us the wait time until the
 * next request can be made cleanup will free the accounting of old requests, should be called periodically
 *
 * @author
 *   piotr 01.03.15 Limitations:
 *   1. Making identical historical data requests within 15 seconds; 2. Making six or more historical data requests for
 *      the same Contract, Exchange and Tick Type within two seconds. 3. Do not make more than 60 historical data
 *      requests in any ten-minute period.
 */
class HistoricalRateLimiter {
  ...
  def requested(request: HistoricalRequest, reftime_ms: Option[Long] = None): Unit = synchronized {
    ...
    while (!requests.put(curTime, request)) {
   ...
   * @param numRequests
   *   the allowed number of requests in this timeframe
   * @param filter
   *   a comparison functor to filter only the given requests if nonEmpty
   * @param reftime_ms
   *   the reference time (now)
   * @return
   *   number of ms after we are able to have numRequests requests or less in the given timeframe with the optional
   *   filter
   */
  def nextSlot_ms(
    timeframe_ms: Long,
  ...
  /**
   * @param request
   *   type of request that we want to make
   * @param reftime_ms
   *   the reference time, what is considered "now", defaults to the current time
   * @return
   *   minimum milliseconds to wait after we can make the next request without violating the limits
   */
