package com.starbright.trade.ibclient

import com.ib.client.Types._
import com.ib.client.{Order => IBOrder, _}
import com.starbright.trade.config.{AppConfig, ConfigLoader}
import com.starbright.trade.errors.{IBApiError, IBClientError}
import com.starbright.trade.ibclient.account.{AccountUpdate, AccountUpdateSubscription, Value}
import com.starbright.trade.ibclient.handler._
import com.starbright.trade.ibclient.order.{ExecutionStatus, Order}
import com.starbright.trade.ibclient.util._
import com.starbright.trade.models.{OrderStatus => _, _}
...
/**
 * The API is fully asynchronous and thread safe. Most of the calls return a Future of the desired result that is
 * eventually completed sucessfully or with an error describing the problem.
 *
 * All the exposed interface is nonblocking, some calls return a [[scala.concurrent.Future]] or
 ...
 *
 * @param host
 *   host where TWS is running
 * @param port
 *   port configured in TWS API settings
 * @param clientId
 *   an integer to identify this client, a duplicated clientId will cause an error on connect
 */
// scalastyle:off
class IBClient(val host: String, val port: Int, val clientId: Int) extends EWrapper {
  private val log: Logger = LoggerFactory.getLogger(this.getClass)

  val config: AppConfig = ConfigLoader()
  ...
  /**
   * A map of request id to Promise Each request with its id is associated with a Handler and a promise to fulfill when
   * the data is ready for consumption by the client.
   */
  val reqHandler = mutable.Map.empty[Int, ErrorHandler]
  ...
  private[this] var orderStatusHandler: OrderStatusHandler = OrderStatusHandler(
    PublishSubject[com.starbright.trade.models.OrderStatus]()
  )
  ...
  val historicalExecutionContext: ExecutionContextExecutor =
   ...
   *
   * @return
   *   A Future[Boolean] that is completed once the client is connected and set to true. If it can't connect it will be
   *   set to false
   * @example
   *   {{{
  ...
  def connect(): Future[IBClient] = synchronized {
    if (eClientSocket.isConnected) {
      ...
      log.info(s"Connecting to Host: $host; Port: $port; with client id: $clientId")
   ...
   * @param contract
   * @param typ
   *   any of ReportSnapshot, ReportsFinSummary, ReportRatios, ReportsFinStatements, RESC, CalendarReport
   * @return
   *   a future string, completed with the data
   */
   ...
   * @param contract
   * @return
   *   a [[MarketDataSubscription]] which contains an Rx observable through which the [[Tick]] is delivered when is
   *   available. This allows to handle market data asynchronously by using reactive programming patterns.
   */
  ...
  /**
   * Close a market data line if there's no subscription with the given id this call has no effect
   *
   * @param id
   *   id of the [[MarketDataSubscription]]
   */
  ...
  /**
   * @return
   *   a future map of orderId to OpenOrder
   */
  ...
  /**
   * @return
   *   an observable of OrderStatus changes
   *   [[https://www.interactivebrokers.com/en/software/api/apiguide/java/orderstatus.htm]]
   */
  def orderStatusObservable(): Observable[com.starbright.trade.models.OrderStatus] =
  ...
  override def orderStatus(
    ...
    orderStatusHandler.subject.onNext(
      com.starbright.trade.models.OrderStatus(
        orderId,
        ExecutionStatus.withName(status),
        filled,
        remaining,
        avgFillPrice,
        permId,
  ...
  /**
   * There can be only one AccountUpdateSubscription, getting more than one will prevent the previous from receiving
   * updates
   *
   * @param account
   *   The account id, empty by default which returns this account
   * @return
   */
  ...
  override def updatePortfolio(
    ...
    log.debug(
      s"updatePortfolio contract: ${contract} position: ${position} mktPrx: ${marketPrice} mktVal: ${marketValue} avgCost: ${averageCost} unrlzPNL ${unrealizedPNL} realizedPNL ${realizedPNL} accountName: ${accountName}"
    )
   ...
   * @param contract
   * @return
   *   contract details for the given contract
   */
  ...
  override def contractDetailsEnd(reqId: Int): Unit = {
    ...
    reqHandler.remove(reqId).foreach { h ⇒
      ...
      reqPromise.remove(reqId).foreach { p ⇒
        ...
        promise.success(contractDetailsHandler.details.toIndexedSeq)
      }
    }
  }
   ...
   *
   * The request is translated internally to potentially several historicalData requests and the future result is
   * fullfilled on successful completion.
   *
   ...
   * @param endDate
   * @param barSize
   *   span of tone bar one of [_1_secs, _5_secs, _10_secs, _15_secs, _30_secs, _1_min, _2_mins, _3_mins, _5_mins,
   *   _10_mins, _15_mins, _20_mins, _30_mins, _1_hour, _4_hours, _1_day, _1_week]
   * @param whatToShow
   *   Determines the nature of data being extracted. One of: [TRADES, MIDPOINT, BID, ASK] for realtime bars and
   *   [BID_ASK, HISTORICAL_VOLATILITY, OPTION_IMPLIED_VOLATILITY, YIELD_ASK, YIELD_BID, YIELD_BID_ASK, YIELD_LAST]
   * @param rthOnly
   *   only data from regular trading hours if true
   * @return
   */
  def easyHistoricalData(
    ...
    val historyRequestAggregation = new Runnable {
      ...
      def doRequest(request: HistoricalRequest): Unit = {
        val barsFuture = historicalData(
          contract,
          request.endDate,
          request.duration,
          historyDuration.durationUnit,
          barSize,
          whatToShow,
          rthOnly,
          false
        )
        Await.ready(barsFuture, Duration(config.historyRequestTimeout))
        barsFuture.value match {
          ...
          // No Data
          case Some(Failure(error: IBApiError))
              if error.code == 162 && error.msg.matches(
      ...
      def run(): Unit = {
        ...
        endDatesDurations.reverseIterator.foreach { dateDuration ⇒
          ...
          val request = new HistoricalRequest(
            contract.symbol,
            contract.exchange,
            endDate,
            duration,
          ...
          val waitTime = scala.concurrent.duration
            .Duration(config.historyRequestPacingViolationRetry.length, config.historyRequestPacingViolationRetry.unit)
          util.retryWhen(config.historyRequestPacingViolationRetry.count)(
            doRequest(request),
            whenAcceptableException,
            waitTime.toMillis
          )
        }
        if (!resultPromise.isCompleted)
   ...
   *
   * This calls once to the underlying EClientSocket, meaning that not all combinations of durationUnit, barSize and
   * duration are legal, plus history requests are limited by
   * [[https://www.interactivebrokers.com/en/software/api/apiguide/tables/historical_data_limitations.htm]]
   *
   * @see
   *   [[util.HistoryLimits]] to retrieve maximum duration for a given combination
   * @param contract
   * @param endDate
   * @param duration
   *   number of durationUnit to request
   * @param durationUnit
   *   time span the request will cover one of [SECOND, DAY, WEEK, MONTH, YEAR] see com.ib.client.Types.DurationUnit
   * @param barSize
   *   span of tone bar one of [_1_secs, _5_secs, _10_secs, _15_secs, _30_secs, _1_min, _2_mins, _3_mins, _5_mins,
   *   _10_mins, _15_mins, _20_mins, _30_mins, _1_hour, _4_hours, _1_day, _1_week]
   * @param whatToShow
   ...
   *   only data from regular trading hours if true
   * @param rateLimit
   *   set to false to skip rate limiting **warning** this can cause pacing violation errors
   * @return
   *   future of IndexedSeq of [[Bar]]
   */
  def historicalData(
    contract: Contract,
    endDate: Date,
    ...
    def doRequest = {
      log.debug(
        s"reqHistoricalData reqId: ${reqId} endDate: ${dateStr} symbol: ${contract.symbol} duration: ${duration}x${durationUnit.toString} barSize: ${barSize}"
      )
   ...
   * @param whatToShow
   * @param rthOnly
   *   use only regular trading hours
   * @return
   *   a [[RealtimeBarsSubscription]] which has an observableBar to which you can subscribe to
   *   {{{
   * val subscription = ibclient.realtimeBars(new CashContract("EUR","EUR.USD"), WhatToShow.MIDPOINT)
  ...
  /**
   * Close realtime bar data line if there's no subscription with the given id this call has no effect
   *
   * @param id
   *   id of the [[RealtimeBarsSubscription]]
   */
  ...
  override def realtimeBar(
    ...
    log.debug(
      s"realtimeBar id: ${reqId} time: ${time} o: ${open} h: ${high} l: ${low} c: ${close} v: ${volume} w: ${wap} cnt: ${count}"
    )
    var handled = false
